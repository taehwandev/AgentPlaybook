...
def _parse_index_descriptions() -> Dict[str, str]:
...
def search_docs(query: str, max_results: int = 8) -> List[Dict[str, object]]:
    """Return docs ranked by keyword relevance to *query*."""
    terms = [t.lower() for t in re.split(r"\s+", query.strip()) if t]
    if not terms:
        return []

    descriptions = _parse_index_descriptions()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    query.add_argument("--max", type=int, default=8, dest="max_results", help="Maximum results to return (default: 8).")
...
def main(argv: List[str]) -> int:
    ...
    if args.action == "query":
        ...
        results = search_docs(query_str, max_results=args.max_results)
        if args.format == "json":
